...
Hey there! This is the third guide in my Modeling Guide with R series. In the previous guide, we looked at simple and multiple linear regression. This page will focus on both logistic and multinomial regression. Although, I cannot cover all of the details and fine use cases of regression, we will explore some of the key ideas to focus on when creating and analyzing a regression model. We'll start by importing our data and identifying variables to use in the model then move to creating, interpreting, and testing the model. In both logistic and multinomial regression, we'll use train/test validation and we'll also look at a few candidate models.

We will be using the same dataset for both the simple and multiple regression cases so let's get that set up first. As mentioned in the previous, guide, I prefer to use the `tidyverse` family of packages. We'll also be using the `tidymodels` collection of packages to set up the models and perform our train/test validation.
...
Although most of the processes for logistic regression will be the same as or very similar to SLR and MLR, there are a few key differences. The key piece is what our response variable looks like. Remember, for SLR and MLR, our response variable had to be continuous. What if you want to predict a variable that isn't continuous? Well, logistic regression may come to the rescue.

## Requirements

In order to get valid results from logistic regression, we must meet certain assumptions.

* The response variable must be categorical with two levels, no more, no less. We might also call this binary.
* All of the observations should be independent
* Our predictor variable(s) should not be correlated with each other
* We must have a linear relationship between the *independent* variable and the *log odds*
* All possible combinations of variables in the model must be represented

Notice that we don't need to assume anything about our errors nor are we required to have a linear relationship between the predictors and the response variable. This is because logistic regression is not predicting the actual value for the response variable, it is predicting how likely it is to obtain a value for the response given the values of the predictors.
...
We should also define our "base" level for customer type or which type we want to compare everything to. I am going to create a true binary variable for gender which takes on a value of 1 if a customer is a member and 0 if they are not. We also need to coerce this variable into a factor in order for the code later on to work.
...
Technically, we can split up logistic regression into simple logistic regression (one predictor variable) and multiple logistic regression (more than one predictor variable). However, I am going to jump right into multiple logistic regression and dive into a model. As I did with MLR, I am going to start by choosing every variable that I think might play a role in predicting the gender of a customer and use the regression output to narrow things down.
...
As mentioned above, the theory behind logistic regression is a little different from linear regression (SLR and MLR).

We are measuring (predicting) the probability of being in a certain class, here whether a customer is a member or not. Since we choose to have "Member" be represented by a 1, we are essentially finding the odds of a certain customer being a member. We can define the odds of a "success" (a customer being member) as:

$$
\frac{Pr(Y_i = 1)}{Pr(Y_i = 0)} = \frac{p_i}{1-p_i}
$$
which is the probability of a given person being a member divided by one minues that same probability.
...
This should look slightly familiar to linear regression! We have a list of predictors and would would like to predict the parameters that allow us to use a function to predict how likely it is that a customer is a member.

Now that we have an understanding of what the logistic model looks like, let's use R to build one!

```{r}
train_subset <- train %>% select(member, store, gender, product_class, pay_method, rating)

log_spec <- logistic_reg() %>%
  set_engine("glm")

log_mod_full <- log_spec %>%
  fit(member ~ store + gender + product_class + pay_method + rating, data = train_subset, family = "binomial")

tidy(log_mod_full)
```

The tricky part about logistic regression is the interpretation. See, we are predicting the *log odds* of obtaining a customer that is a member. So, each value in the `estimate` column above can be interpreted as the change in the log odds of finding a member for a unit increase in the respective $X$ variable.

So if a customer shops at Store C, their predicted log odds of being a member increases by 0.123 and therefore their predicted odds of being a member increases by $e^{0.123} = 1.13$.

One thing to notice here is that none of the p-values for the estimates are statistically significant at the "typically" 0.05 level. It is important to note the difference between statistical significance and practical significance. It may be that we don't meet the 0.05 requirement, but sometimes, the p-value may be small enough to still mean something to us. At this moment, we are looking at the different variables that may help us predict if someone is a member or not. We don't necessarily need to have statistical significance as long as we are still getting good insight into our question.

We can also explore our variables a little bit further. Using the following plot, we can look at the distribution of each of our variables separated by the member status of a customer.
...
With that being said, I would like to at least try a reduced model to see what the results are. I will use the p-values from above to guide my decision making and to create a reduced model. Some of these variables are not very useful at predicting member status so let's remove them from the model and try again. Let's use just payment method and product class.

```{r}
log_mod_reduced <- log_spec %>%
  fit(member ~ product_class + pay_method, data = train_subset, family = "binomial")

tidy(log_mod_reduced)
```

This model isn't much better but we do see some evidence that certain product classes and payment methods are more associated with a member than otherwise. For instance, if a customer purchases a food/beverage or a sports/travel item, their log odds of being a member increases by about 0.33. Perhaps members get a slight discount on items in these areas so they purchases more.

We don't seem to do much better than the full model but there are some small improvements. Let's stick with this reduced model moving forward.
...
This plot shows us how well the model did. It compares the actual member status with what the model would predict a customer to be. I did add a little bit of jitter to the plot so we could see each observation better (otherwise they all could lie on top of each other).

The ideal situation is 100% prediction accuracy where all of the points lie at (0, 0) meaning we predicted a customer to not be a member and they actually weren't or at (1, 1) meaning we predicted a customer to be a member and they actually were. In the real world, we will have some error and will misclassify some observations which appear in (0, 1) and (1, 0). Looking at this plot, we see that we did in fact predict the member status correctly for many customers! However there are quite a few observations that we did not predict correctly.
...
We see that we got $181 + 257 = 438$ observations correctly predicted which means that we incorrectly predicted 361. We were $\frac{438}{799} * 100 = 54.82\%$ accurate. Not much better than just randomly guessing.

I also want to check our prediction accuracy with the testing dataset.
...
We see similar results from the training data. We did correctly predict some observations, but there are still quite a handful that we missed. To quantify it, we correctly predicted 103 out of 201 customer member statuses which gives an accuracy rate of 51.24%.

Let's discuss our assumptions:

* The response variable must be categorical with two levels, no more, no less. We might also call this binary.

We met this when we chose variables earlier.
...
* We must have a linear relationship between the and the *log odds* the predictor variables
